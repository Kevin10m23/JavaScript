...
function liquidar(){
    ...
    document.getElementById("basico").value=sueldo.toFixed(0);//subir del vscode al form

    //SUBSIDIO TRANSPORTE 
    if(n1>2*smlv){
        subsidiot=0; // no se otorga subsidio
        document.getElementById("subsidio").value=subcidiot.toFixed(0);
    }else{
...
//RECARGO NOCTURNO
    ...
    //
    let totaldevo=document.getElementById("toaldev").value;
    ...
    //PRESTAMO 
    let presta=document.getElementById("fondos").value;
    //TOTAL DEDUCCIONES 
    ...
    //
    let totalddc=document.getElementById("toaldedu").value;
    //CALCULAR NETO PAGO
